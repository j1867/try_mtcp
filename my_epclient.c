...
static inline int 
SendData(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
	char data[] = "1";
	struct mtcp_epoll_event ev;
	int wr;
	...
	wr = mtcp_write(ctx->mctx, sockid, data, len);
	if (wr < len) {
...
main(int argc, char **argv)
{
	...
	for (i = ((process_cpu == -1) ? 0 : process_cpu); i < core_limit; i++) {
		...
		if (pthread_create(&app_thread[i], 
					NULL, RunSendDataMain, (void *)&cores[i])) {
